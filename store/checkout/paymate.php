...
// CHECK IF RESPONSE WAS FROM GATEWAY..
if (isset($_GET['callback'])) {
  $order = $GATEWAY->pingHandler();
  if (isset($order[0], $order[1])) {
    header("Location: " . $SETTINGS->ifolder . '/index.php?gw=' . $order[0] . '-' . $order[1]);
  } else {
    header("Location: " . $SETTINGS->ifolder . '/index.php?p=error&msg=Callback failure');
  }
  exit;
}
    ...
    if ($GATEWAY->validateResponse($PARAMS, $SALE_ORDER) == 'ok') {
      ...
      if ($SALE_ORDER->saleConfirmation == 'no') {
        ...
        switch($paymentStatus) {
          ...
          // NOT currently supported by Paymate
          //==========================================
