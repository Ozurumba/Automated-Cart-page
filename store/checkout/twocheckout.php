...
if ($INCOMING['code-id']) {
  ...
  if (isset($SALE_ORDER->id) && $MARKER == 'mswcart') {
    ...
    if ($GATEWAY->validateResponse($PARAMS, $SALE_ORDER) == 'ok') {
      ...
      // ADJUST FOR REFUNDED..
      if ($paymentStatus == 'refund_issued') {
        $paymentStatus                = 'refunded';
        $SALE_ORDER->saleConfirmation = 'no';
      }

      // ADJUST FOR COMPLETED SALE..
      if ($paymentStatus == 'fraud_status_changed' && strtolower($INCOMING['fraud-status']) == 'pass') {
        $paymentStatus = 'completed';
      }

      // ADJUST FOR PENDING SALE (ORDER CREATED)..
      if ($paymentStatus == 'order_created' && strtolower($INCOMING['fraud-status']) == 'wait' && strtolower($INCOMING['inv-status']) == 'approved') {
        if ($SETTINGS->pendingAsComplete == 'yes') {
          $paymentStatus = 'completed';
        } else {
