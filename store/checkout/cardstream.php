    ...
    if ($GATEWAY->validateResponse($PARAMS, $SALE_ORDER) == 'ok') {
      ...
      if ($SALE_ORDER->saleConfirmation == 'no') {
        ...
        switch($paymentStatus) {
          ...
          case 'completed':
            if (($INCOMING['amount'] >= str_replace('.', '', $SALE_ORDER->grandTotal)) && ($INCOMING['currency'] == $SETTINGS->baseCurrency)) {
          ...
          // NOT currently supported by Cardstream
          //==========================================
          ...
          default:
            ...
            if (isset($_POST['responseMessage'])) {
              $GATEWAY->writeLog($SALE_ID, 'Gateway message: ' . $_POST['responseMessage']);
            }
            break;
        }
      } else {
