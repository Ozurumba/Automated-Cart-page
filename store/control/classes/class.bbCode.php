...
class bbCode_Parser {
  ...
  public function bbParser($text) {
    ...
    if (strpos($text, '[') === false && strpos($text, ']') === false) {
      return (AUTO_PARSE_LINE_BREAKS ? mc_NL2BR(htmlspecialchars($text)) : htmlspecialchars($text));
    }
    $tagList = array(
      '[b]' => '<span class="bbBold">',
    ...
    $text    = bbCode_Parser::dailyMotionParser($text);
    // Parse soundcloud videos..
    $text    = bbCode_Parser::soundCloudParser($text);
    // Parse mp3 files..
    $text    = bbCode_Parser::mp3Parser($text);
    // Parse emails..
    ...
    $text    = (AUTO_PARSE_LINE_BREAKS ? mc_NL2BR(trim($text)) : trim($text));
