...
class mcDownload {
  ...
  // Enable download log..
  // Folder in store root and writeable..
  // File has unique identifier added to start if it exists..
  private $log = array(
    'enable' => 'no',
    'folder' => 'logs',
    'file' => 'download-log.log'
  );
  ...
  public function mime($file, $mime) {
    ...
    $a = mcDownload::browser();
    $t = mcDownload::mime_types();
  ...
  public function dl($path, $mime, $delete = 'no') {
    ...
    if (!file_exists($path)) {
      die('<b>' . $path . '</b> does NOT exist and cannot be downloaded. Check destination exists and is writeable.');
    }
    ...
    /*
    if ($mime != 'application/force-download') {
  ...
  public function log($msg) {
    if ($this->log['enable'] == 'yes') {
      $message  = 'Date/Time: ' . date("j F Y @ H:i:A") . mc_defineNewline();
      if (isset($_SESSION['dl-log-cntr-ip'])) {
        $message  .= 'IP(s) ' . $_SESSION['dl-log-cntr-ip'] . mc_defineNewline();
      }
      $message .= $msg . mc_defineNewline();
      $message .= mc_defineNewline() . '= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =' . mc_defineNewline() . mc_defineNewline();
      // Attempt to create log directory if it doesn`t exist..
      if (!is_dir(PATH . $this->log['folder'])) {
        $oldumask = @umask(0);
        @mkdir(PATH . $this->log['folder'], 0777);
        @umask($oldumask);
      }
      if (is_dir(PATH . $this->log['folder']) && is_writeable(PATH . $this->log['folder'])) {
        @file_put_contents(PATH . $this->log['folder'] . '/' . (isset($_SESSION['dl-log-cntr']) ? $_SESSION['dl-log-cntr'] : '') . '-' . $this->log['file'], $message, FILE_APPEND);
      }
    }
  }
