...
if (!function_exists('password_hash')) {
  die('Software Load Failure.<br><br>PHP <b>must</b> be compiled with <a href="http://php.net/manual/en/book.password.php">password hash API</a> support.');
}
...
define('LIC_UNI', '87007AF51BF92A0435D05A30723A29CB8AF781AD');
define('LIC_ENC_SALT', 'wQdu9Eej3ARoge8tZJq4Mm16WfOpyMb3TNqkoKzdg8Irn8pU6vR26xvbYB0t3LWUv5YMI8Ddyw73jUT5EL6kFg==');
define('LIC_SOFTWARE', 'cart');
define('RESTR_CATS', 20);
define('RESTR_PROD', 100);
define('RESTR_USERS', 2);
define('RESTR_PICS', 5);
define('RESTR_PERS', 2);
define('RESTR_ATTR', 5);
define('RESTR_ATTR_GRP', 2);
define('RESTR_COUPONS', 2);
define('RESTR_POINTS', 5);
define('DEV_BETA', 'no');
...
class mswLic {
  ...
  public function mswCheckLicence() {
    if (@file_exists(LIC_PATH . 'licence.lic')) {
      $Q       = @mysqli_query($GLOBALS["___msw_sqli"], "SELECT `prodKey` FROM `" . DB_PREFIX . "settings`");
      if ($Q == 'err') {
        $fatalErr = true;
      } else {
  ...
  public function mswEventHandler($code, $domain = '', $key = '', $s = '', $exp = 0) {
    ...
    switch($code) {
      ...
      case '3':
        ...
        break;
      case '4':
        $e = 'This beta version expired on ' . date('j/M/Y', $exp) . '. All beta versions are valid for 1 month only.<br><br>If you are an active beta tester, please contact us for a new licence file.<br><span style="font-weight:normal"><a href="mailto:msworld67@gmail.com?subject=Beta%20Licence">msworld67@gmail.com</a></span><br><br>Remember that beta versions should NOT be used in a production environment.';
        break;
      case '5':
    ...
    $footer  = '<p class="footer"><a href="https://www.maiangateway.com" onclick="window.open(this);return false">Maian Script World Licencing System</a> &copy;2006 -' . date('Y') . ' David Ian Bennett &amp; Maian Script World</p>';
  ...
  public function mswSysCheck() {
    $err = array();
    $cnt = 0;
    $Q   = @mysqli_query($GLOBALS["___msw_sqli"], "SELECT @@sql_mode AS `mode`");
    ...
    if (phpVersion() < MSW_PHP_MIN_VER) {
    ...
    } elseif (isset($ST->mode) && strpos(strtolower($ST->mode), 'strict_trans_tables') !== false) {
      $err[] = '(' . (++$cnt) . ') This software does not currently support MySQL running in STRICT mode. Please disable to continue.';
    } else {
      if (is_dir(LIC_PATH . 'install')) {
        $err[] = '(' . (++$cnt) . ') Please remove or rename the "install" folder in your helpdesk store directory.';
      }
      if (DEV_BETA_EXP == '0000-00-00') {
        if (file_exists(LIC_PATH . 'patch.php')) {
          $err[] = '(' . (++$cnt) . ') Please remove the "patch.php" file from your installation.';
        }
      }
      if (SECRET_KEY == 'abc12345') {
      ...
      if (!function_exists('imagecreatefromjpeg')) {
        $err[] = '(' . (++$cnt) . ') <a href="http://php.net/manual/en/book.image.php" onclick="window.open(this);return false">GD GRAPHICS LIBRARY</a> NOT enabled. Please recompile server with gd support.';
      }
      ...
      if (!class_exists('ZipArchive')) {
        $err[] = '(' . (++$cnt) . ') <a href="http://php.net/manual/en/class.ziparchive.php" onclick="window.open(this);return false">ZIPARCHIVE</a> class NOT enabled. Please recompile server with zipArchive support.';
      }
    }
    if (!empty($err)) {
      $doctype = '<!DOCTYPE html><html lang="en">';
      $footer  = '<p class="footer"><a href="https://www.' . SCRIPT_URL . '" onclick="window.open(this);return false">' . SCRIPT_NAME . '</a> &copy;2006 -' . date('Y') . ' David Ian Bennett &amp; Maian Script World</p>';
  ...
  public function mswSysFreeCheck() {
    $err = array();
    $cnt = 0;
    if (mc_rowCount('categories') > RESTR_CATS) {
      $err[] = '(' . (++$cnt) . ') The free version permits the following number of categories: ' . RESTR_CATS;
    }
    if (mc_rowCount('products') > RESTR_PROD) {
      $err[] = '(' . (++$cnt) . ') The free version permits the following number of products: ' . RESTR_PROD;
    }
    if (mc_rowCount('users') > RESTR_USERS) {
      $err[] = '(' . (++$cnt) . ') The free version permits the following number of users: ' . RESTR_USERS;
    }
    if (mc_rowCount('campaigns') > RESTR_COUPONS) {
      $err[] = '(' . (++$cnt) . ') The free version permits the following number of coupon campaigns: ' . RESTR_COUPONS;
    }
    if (mc_rowCount('price_points') > RESTR_POINTS) {
      $err[] = '(' . (++$cnt) . ') The free version permits the following number of price points: ' . RESTR_POINTS;
    }
    $rt = 0;
    $q = mysqli_query($GLOBALS["___msw_sqli"], "SELECT count(*) AS `c` FROM `".DB_PREFIX."pictures` GROUP BY `product_id` ORDER BY 1 DESC")
         or die(mc_MySQLError(__LINE__,__FILE__));
    while ($R = mysqli_fetch_object($q)) {
      if ($R->c > RESTR_PICS) {
        $err[] = '(' . (++$cnt) . ') The free version permits the following number of pictures per product: ' . RESTR_PICS;
        ++$rt;
        break;
      }
      if ($rt > 0){
    ...
    $rt = 0;
    $q = mysqli_query($GLOBALS["___msw_sqli"], "SELECT count(*) AS `c` FROM `".DB_PREFIX."attr_groups` GROUP BY `productID` ORDER BY 1 DESC")
         or die(mc_MySQLError(__LINE__,__FILE__));
    while ($R = mysqli_fetch_object($q)) {
      if ($R->c > RESTR_ATTR_GRP) {
        $err[] = '(' . (++$cnt) . ') The free version permits the following number of attribute groups per product: ' . RESTR_ATTR_GRP;
        ++$rt;
        break;
      }
      if ($rt > 0){
    ...
    $rt = 0;
    $q = mysqli_query($GLOBALS["___msw_sqli"], "SELECT count(*) AS `c` FROM `".DB_PREFIX."attributes` GROUP BY `attrGroup` ORDER BY 1 DESC")
         or die(mc_MySQLError(__LINE__,__FILE__));
    while ($R = mysqli_fetch_object($q)) {
      if ($R->c > RESTR_ATTR) {
        $err[] = '(' . (++$cnt) . ') The free version permits the following number of attributes per product: ' . RESTR_ATTR;
        ++$rt;
        break;
      }
      if ($rt > 0){
    ...
    $rt = 0;
    $q = mysqli_query($GLOBALS["___msw_sqli"], "SELECT count(*) AS `c` FROM `".DB_PREFIX."personalisation` GROUP BY `productID` ORDER BY 1 DESC")
         or die(mc_MySQLError(__LINE__,__FILE__));
    while ($R = mysqli_fetch_object($q)) {
      if ($R->c > RESTR_PERS) {
        $err[] = '(' . (++$cnt) . ') The free version permits the following number of personalisation options per product: ' . RESTR_PERS;
        ++$rt;
        break;
      }
      if ($rt > 0){
    ...
    if (!empty($err)) {
      ...
      + 3 Months personal support (renewable)</span><br>Click the purchase link in your admin area for more information.<br><br>I apologise for any inconvenience.<br><br>David Ian Bennett<span class="leaddev">(Lead Developer - Maian Script World)</span>';
